...
class RedisSessionImpl implements RedisSession {
  ...
  RedisSessionImpl(AbstractRedisClient client, StatefulConnection connection, RedisClusterAsyncCommands<byte[], byte[]> asyncCommands, RedisSinkConnectorConfig config) {
    this.client = client;
